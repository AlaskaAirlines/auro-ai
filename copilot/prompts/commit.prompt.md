...
<!-- Generated from plugins/auro/skills/commit/SKILL.md by scripts/build-copilot-prompts.mjs. Do not edit by hand. -->

> **Argument** (`${input:args}`): "<ADO ticket # | PR # | prev | amend>"
...
**The invocation takes one argument** — available as `${input:args}` (the text after `/commit`, trimmed; empty if none):
...
Inspect `${input:args}` (trimmed, case-insensitive):
...
The reference comes from **`${input:args}`** — do **not** prompt for it in the normal case. Resolve it as follows.

**1. `prev` / `previous`.** If `${input:args}` (trimmed, case-insensitive) is `prev` or `previous`, reuse the reference from the last time `/commit` was run in this repo. Read it with:
...
**2. An explicit number.** Otherwise classify `${input:args}` by its **character length** (trim whitespace; strip a leading `#` or `AB#` if present, then measure the remaining digits):
...
**3. Missing or invalid — the reference is required.** If `${input:args}` is empty, or is 8+ characters, or is not numeric after stripping, the skill cannot proceed without a reference. Prompt with a plain-text message:
