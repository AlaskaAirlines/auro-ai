...
**1. `prev` / `previous`.** If `$ARGUMENTS` (trimmed, case-insensitive) is `prev` or `previous`, reuse the reference from the last time `/commit` was run in this repo. Read it with:
...
**2. An explicit number.** Otherwise classify `$ARGUMENTS` by its **character length** (trim whitespace; strip a leading `#` or `AB#` if present, then measure the remaining digits):
