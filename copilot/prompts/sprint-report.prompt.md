...
<!-- Generated from plugins/auro/skills/sprint-report/SKILL.md by scripts/build-copilot-prompts.mjs. Do not edit by hand. -->

> **Copilot compatibility:** Requires an Azure DevOps PAT in your environment and shell tools (`curl`, `jq`); the terminal will ask to approve each command.
...
A flat WIQL query returns only work item **IDs**, so the report is built in stages: run the saved query for the IDs, batch-fetch each item's `System.WorkItemType`, `System.State`, `System.ChangedDate`, `System.AssignedTo`, `System.AreaPath`, `System.CreatedDate`, `System.Tags`, `System.Title`, `Microsoft.VSTS.Scheduling.StoryPoints` (User Story points), and `Microsoft.VSTS.Common.Severity` (Bug severity) (the batch endpoint accepts at most 200 IDs per call, so the IDs are chunked), then — for the **Closed** items in **either** the current or previous sprint's range — a second batch pass fetches their **relations** to detect whether each has any linked GitHub **pull request or commit**. Because the saved query returns the whole board (not just one sprint), both sprints are computed from the same fetched rows: items whose **changed date falls within `[START, FINISH]` inclusive** (comparing the `YYYY-MM-DD` date portion, so the entire finish day counts) feed the current matrix, and items within `[PSTART, PFINISH]` feed the previous-sprint counts. Immediately after the fetch, a **report-wide type filter** removes the `Test Case`, `Test Plan`, `Test Suite`, `Epic`, `Feature`, and `Initiative` work item types from the shared rows file, so they appear in **none** of the eleven tables (every pass reads the already-filtered file).

Four awk passes, two snapshot **done** passes, one **revision-history** pass, and one **completion** pass build **eleven** tables. The first pass emits blocks marked `=== CURRENT ===` and `=== DIFF ===`:
...
# 2. Batch-fetch id + type + state + changed date + assignee + area path + created date + tags + title +
#    story points + severity in chunks of 200 ->
#    "id<TAB>changed<TAB>type<TAB>state<TAB>assignee<TAB>area<TAB>created<TAB>tags<TAB>title<TAB>storypoints<TAB>severity"
#    (assignee display name or "Unassigned"; tags is ADO's "; "-joined tag string, empty when the item has no tags;
#    title has any embedded tabs/newlines squashed to spaces; storypoints is Microsoft.VSTS.Scheduling.StoryPoints
#    (User Stories) as a bare number or empty; severity is Microsoft.VSTS.Common.Severity (Bugs), e.g. "2 - High",
#    empty when unset). Title is field 9 with two clean trailing columns after it, so every row is 11 tab-columns.
: > /tmp/sprint_rows.tsv
IDS_JSON=$(jq -c '[.workItems[].id]' /tmp/sprint_wiql.json)
TOTAL=$(jq 'length' <<<"$IDS_JSON")
for S in $(seq 0 200 $((TOTAL-1))); do
  BODY=$(jq -c --argjson s "$S" '{ids: .[$s:$s+200], fields:["System.WorkItemType","System.State","System.ChangedDate","System.AssignedTo","System.AreaPath","System.CreatedDate","System.Tags","System.Title","Microsoft.VSTS.Scheduling.StoryPoints","Microsoft.VSTS.Common.Severity"]}' <<<"$IDS_JSON")
  curl -sS -u ":$ADO_PAT" -X POST -H "Content-Type: application/json" --data-binary "$BODY" \
    "$BASE/workitemsbatch?api-version=7.0" \
  | jq -r '.value[] | "\(.id)\t\(.fields["System.ChangedDate"][:10])\t\(.fields["System.WorkItemType"])\t\(.fields["System.State"])\t\(.fields["System.AssignedTo"].displayName // "Unassigned")\t\(.fields["System.AreaPath"])\t\(.fields["System.CreatedDate"][:10])\t\(.fields["System.Tags"] // "")\t\((.fields["System.Title"] // "") | gsub("[\t\n\r]"; " "))\t\((.fields["Microsoft.VSTS.Scheduling.StoryPoints"] // "") | tostring)\t\((.fields["Microsoft.VSTS.Common.Severity"] // "") | gsub("[\t\n\r]"; " "))"' >> /tmp/sprint_rows.tsv
done
echo "fetched rows: $(wc -l </tmp/sprint_rows.tsv | tr -d ' ') of $TOTAL"

# 2b. Report-wide type filter: drop these work item types so they appear in NONE of the eleven tables. Applied
...
#    moment in [START,FINISH], per its revision history) whose CURRENT state is a still-open WORKED state -- i.e.
#    NOT New, Approved, Closed, Removed, Rejected, or Done (New and Approved are not WIP states, so an item parked
#    back in New/Approved is excluded even if it was worked mid-sprint). Revision history is a PER-ITEM call
#    (GET .../workitems/{id}/revisions -- not batchable), so the snapshot decides every item it can and only the
#    rest fetch history:
#      * auto-INCLUDE: last ChangedDate <= FINISH (the current worked state provably reaches into the window).
#      * else (changed after FINISH): fetch revisions to confirm a worked state was held during the window.
: > /tmp/sprint_wip_autoinc.txt; : > /tmp/sprint_wip_needhist.txt; : > /tmp/sprint_wip_qualify.txt
awk -F'\t' -v lo="$START" -v hi="$FINISH" '
{ id=$1; d=$2; s=$4
  # current state must be an open, worked state -- New/Approved are not WIP; Closed/Removed/Rejected/Done are gone
  if(s=="New"||s=="Approved"||s=="Closed"||s=="Removed"||s=="Rejected"||s=="Done") next
  if(d<=hi){ print id > "/tmp/sprint_wip_autoinc.txt"; next }         # worked state provably reaches into the window
  print id > "/tmp/sprint_wip_needhist.txt"                           # changed after FINISH -> confirm via history
}' /tmp/sprint_rows.tsv
...
# 9. Build the COMPLETION breakdown (the eleventh, final table): over the SAME set as the eighth table
#    (DONE WITH CODE) -- tickets CHANGED this sprint whose CURRENT state is a done state (Closed/Done/Rejected)
#    AND that carry at least one linked GitHub commit or pull request (exactly the ids already computed into
#    /tmp/sprint_done_code_ids.txt above). Summarize the delivered work as a flat Metric/Value list: the count
#    of completed User Stories and the sum of their Story Points (field $10, velocity), then the count of
#    resolved Bugs broken down by Severity (field $11: "1 - Critical" / "2 - High" / "3 - Medium" / "4 - Low";
#    anything else counts as Unspecified) with a Total. Reads the done-with-code id list as a keep-list (NF==1)
#    plus /tmp/sprint_rows.tsv (11 tab-columns); the keep-list already encodes the window+state+linked-code
#    predicate, so membership is just "id in keep". Prints NO_COMPLETION_ITEMS if no story/bug shipped code.
echo "=== COMPLETION ==="
awk -F'\t' '
NF==1 { keep[$1]=1; next }
NF>=11 && ($1 in keep) {
  t=$3; sp=$10; sev=$11
  if(t=="User Story"){ us++; if(sp!="") pts+=sp }
  else if(t=="Bug"){
    bugs++
...
if [ ! -s /tmp/sprint_rca_ids.txt ]; then
  ...
  #    dates, contributors) are pre-assembled here exactly the way the eleven data tables are printed as
...
**Eleventh table — sprint completion breakdown** (the `=== COMPLETION ===` block). A small two-column **Metric / Value** table summarizing what was actually delivered this sprint, scoped to the same items as the eighth table (`DONE WITH CODE`) — tickets changed this sprint whose current state is Closed/Done/Rejected **and** that have a linked GitHub commit or pull request. Tickets closed without linked code are deliberately excluded, so this reflects only work that shipped code. It reports **User Stories completed** and their summed **Story Points completed** (sprint velocity), then **Bugs resolved** broken down by severity — **Critical**, **High**, **Medium**, **Low**, an **Unspecified severity** row only when non-zero, and a **Total**. Render the rows verbatim in the printed order; there is no totals column to bold, but bold the `Bugs resolved (Total)` row's label for readability. If Step 3 printed `NO_COMPLETION_ITEMS`, note that no story or bug was completed this sprint and skip the table. Head it, e.g.:

> **Sprint 17.26** (2026-08-12 → 2026-08-25) — completion breakdown (stories & bugs finished this sprint)

*What this shows: the sprint's delivered output at a glance — how many story points of planned work the team completed, and how many bugs it cleared by severity. Useful for tracking velocity and the balance between feature work and defect burn-down.*

| Metric | Value |
|---|---:|
| User Stories completed | 7 |
| Story Points completed | 34 |
| Bugs resolved (Critical) | 1 |
| Bugs resolved (High) | 3 |
| Bugs resolved (Medium) | 5 |
| Bugs resolved (Low) | 2 |
| **Bugs resolved (Total)** | 11 |

Follow with a one-line summary. Note that counts reflect items **changed during the iteration's date range**, not the whole board; that State is each item's **current** state (not its state during the sprint); that the `New` state is split into `New` (items whose **created date** falls inside the sprint — genuinely new) and `New (edit only)` (items still in New that were created before the sprint), with the split computed per-sprint in the diff tables; that `Closed (PR/commit)` means the item has at least one linked GitHub pull request or commit (a linked GitHub *issue* alone does not count as code); that the Support-tagged tables (3 and 4) are the same as tables 1 and 2 filtered to items whose **current** tags include `Support`, so they show only the states that occur within that subset; that the diff tables span the **union** of rows/states seen in either sprint (so a row absent this sprint can still show a negative); that assignee reflects the item's **current** `Assigned To` (unassigned items are grouped as `Unassigned`, and a person may appear under more than one name spelling if the board records them inconsistently); and that Area Path rows are the item's **current** area path with the `E_Retain_Content\Auro Design System` prefix trimmed (items on that top-level node shown as `(root)`), sorted alphabetically. Note also that the tenth table (open work-in-progress) is the **only** one that uses **revision history** rather than the current snapshot: it lists tickets that held a state other than New/Approved at any moment inside the sprint window and whose current state is a still-open worked state (not New, Approved, Closed, Removed, Rejected, or Done) — so it surfaces items worked mid-sprint that other tables (keyed on current state) may otherwise summarize only by their latest state, while still excluding anything now parked back in New/Approved. The eighth and ninth tables (done with/without linked code) instead use the current snapshot: tickets changed in the window whose current state is Closed/Done/Rejected, split by whether they carry a linked GitHub commit or PR. Section 8a (the bug root-cause narrative, rendered right after the eighth table) traces each closed bug that shipped code back through `git blame` to the prior PR(s) and contributors that last touched the fixed lines — flagging Support tickets and their reporter, naming who reviewed that last touch (humans vs AI bots), giving an explicitly-labeled AI-vs-human authorship guess, a leadership-level why-it-slipped-through process assessment, the regression remedies the fix already applied, and follow-on recommendations — and closes with a "how the team can ship more reliably" summary. It's an interpretive aid, so its guesses are flagged as guesses and its scope (bugs with linked code only) is stated. The eleventh table (completion breakdown) summarizes the delivered work over the **same set as the eighth table** (done this sprint **with** linked code — tickets closed without a linked commit or PR are excluded): completed User Story count and summed Story Points (velocity), and resolved Bug counts by Severity — so its User Story and Bug numbers reconcile with the eighth table's rows. Don't editorialize beyond the numbers.
