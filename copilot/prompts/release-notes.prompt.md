...
<!-- Generated from plugins/auro/skills/release-notes/SKILL.md by scripts/build-copilot-prompts.mjs. Do not edit by hand. -->

> **Argument** (`${input:args}`): "[base ref]"
...
**The invocation takes one optional argument** — available as `${input:args}` (the text after `/release-notes`, trimmed; empty if none): an explicit **base ref** (tag, branch, or commit) to use as the start of the commit range, overriding the automatic boundary detection in Step 2. Leave it empty for the normal flow.
...
**If `${input:args}` is non-empty**, treat it as an explicit base ref and use the range `<${input:args}>..HEAD` (verify it resolves with `git rev-parse --verify <ref>`; if it doesn't, tell the user the ref is invalid and stop). Skip the automatic detection below.
