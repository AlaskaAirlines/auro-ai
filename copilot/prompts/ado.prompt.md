...
tools: ['runCommands', 'editFiles', 'fetch']
---

<!-- Generated from plugins/auro/skills/ado/SKILL.md by scripts/build-copilot-prompts.mjs. Do not edit by hand. -->

> **Argument** (`${input:args}`): "new | <ADO ticket #>"

> **Copilot compatibility:** Requires the GitHub CLI (`gh`) and an Azure DevOps PAT in your environment; the terminal will ask to approve each command.
...
`${input:args}` = text after `/ado`, trimmed. A mode argument is **required**: `new` → **create mode**; a ticket number (digits, optional `#`/`AB#`) → **edit mode**.
...
1. Determine mode from `${input:args}`:
