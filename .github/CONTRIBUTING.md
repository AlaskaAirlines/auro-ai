...
1. BEFORE submitting an issue, make sure that you are **using the latest version** of the project you are working with.
1. Check if the issue has already been reported. Go to the repo's issue page and use Github's search features to see if a similar issue has been submitted.
...
A good bug report shouldn't leave others needing to follow up for more information. Please try to be as detailed as possible in your report. What is your environment? What steps will reproduce the issue? What would you expect to be the outcome? All these details will help people to fix any potential bugs.
...
Please add all details to the commit description in the body of the commit message. There is no character limit and no need to be imperative. Listing out the files that were changed in the commit is always helpful.
