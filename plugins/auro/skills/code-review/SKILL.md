...
You are executing the **code-review** skill. The invocation itself is the request: **begin the review immediately and autonomously.** Do not treat the text below as reference documentation — it is your procedure to follow now. Do not ask the user what they want, **with four sanctioned prompts and no others:** (1) **the effort-level prompt** — once per run, just before the reviewers fan out, state the recommended review effort and let the user accept it or force a different level (see "Choose the review effort level"); (2) in local mode, the single base-branch question described below, asked first; (3) in PR mode, after the review is complete, the single submit-or-exit question described in "Preview and confirm before posting (PR mode)" before any comment is posted to GitHub; and (4) in PR mode, **only after** the user has chosen to submit, the single acceptance-criteria-update question described in "Update ADO acceptance criteria (PR mode, opt-in)" — asked at most once and **skipped silently** when there is no resolved acceptance criterion to mark. In practice **at most three** of these fire in any one run — local mode asks (2) then (1) (at most two); PR mode asks (1), then (3), and then (4) only when the user submitted **and** there is at least one resolved AC to mark — and no others. These are the **only** questions this skill may ever ask. In particular, **never ask the user which model(s) to use or whether to run single- vs multi-model** — multi-model is always on and non-negotiable (see "Multi-model review"); there is no single-model mode and no such choice to offer. Silently run the fixed two-model roster (Opus 4.8 + Sonnet 5). The effort prompt sets only the *reasoning effort* each reviewer runs at — it never changes which models run.
...
**PR mode only — skip this section entirely if `$ARGUMENTS` is empty or "local".** This is the submit-or-exit question — the third sanctioned prompt (see "Task — start now"); the only prompt that may follow it is the optional post-submit acceptance-criteria question in "Update ADO acceptance criteria (PR mode, opt-in)". Nothing is written to GitHub before the user confirms here — until then, a PR review behaves exactly like a local review.
...
1. **The reply is `submit`** (case-insensitive; also treat an obvious affirmative equivalent like `post`, `yes`, `y`, or `go` this way) → proceed to "Posting comments (GitHub mode only)" below and perform all GitHub writes (description sync, inline comments, summary). Then, once the GitHub writes are done, proceed to "Update ADO acceptance criteria (PR mode, opt-in)" — the last remaining PR-mode prompt, which is itself skipped silently when there is no resolved AC to mark.
...
## Update ADO acceptance criteria (PR mode, opt-in)

**PR mode only — skip this section entirely if `$ARGUMENTS` is empty or `local`.** This step runs **only** after the user replied `submit` in "Preview and confirm before posting (PR mode)" and **only after** all the GitHub writes above are complete. Its prompt is the **fourth** sanctioned prompt (see "Task — start now") and the last thing a PR-mode run may ask. It writes to Azure DevOps, not GitHub — never run it in local mode, and never run it when the user chose `exit` (or anything non-`submit`) at the submit gate.

**Gate — decide silently whether there is anything to offer.** Before asking anything, determine whether any acceptance criterion can be marked. **Ask nothing and skip this section silently** (no prompt, no note beyond an optional one-liner) when **any** of these hold:
- The commits reference **no** `AB#` ticket (there is no work item to update).
- `ADO_PAT` is unset, or every ADO fetch in the pre-review step failed / bounced to the sign-in page (you could not read the ticket, so you cannot reliably write it).
- No linked ticket exposes a non-empty `Microsoft.VSTS.Common.AcceptanceCriteria` field.
- "Validate ticket completeness" classified **no** requirement as **Resolved** for any linked ticket — i.e. this PR does not fully fulfill any acceptance criterion. (Partial and Not-resolved criteria are never marked.)

Only when at least one linked ticket has acceptance-criteria content **and** at least one of its criteria was classified **Resolved** by this diff do you proceed to the prompt.

**Prompt.** Ask with a plain-text message (**not** the `AskUserQuestion` tool — in a forked skill run it does not surface an interactive prompt, so the ask would be silently skipped, exactly as for the other prompts). Name the tickets and the count of criteria you would mark. Do **not** tell the user to "press enter" — every reply must be non-empty. Ask, for example:

> The review has been posted to PR #$ARGUMENTS. This change appears to fully satisfy **<N>** acceptance criteria (<AB#…, AB#… — the tickets with resolved ACs>). Reply `yes` to mark those criteria complete in the ADO ticket(s), or `no` to leave the ticket(s) unchanged.

Then interpret the reply:
1. **Affirmative** (`yes`, `y`, `ok`, `go`, or a clear equivalent) → perform the updates below.
2. **Anything else** (`no`, `skip`, `cancel`, or anything unrecognized) → make **no** ADO write. Output "🎫 ADO acceptance criteria left unchanged." and stop. Defaulting an ambiguous reply to *no write* is deliberate — the ticket is a shared record and should only change on explicit confirmation.

**Which criteria to mark.** Mark **only** the acceptance criteria that "Validate ticket completeness" classified as **Resolved** by this diff — never Partial or Not-resolved ones. Map each Resolved requirement back to the specific line/item it came from in that ticket's `Microsoft.VSTS.Common.AcceptanceCriteria` field. When a Resolved requirement was derived by splitting a compound AC line, only mark that line if **every** requirement it produced is Resolved; otherwise leave it (it is still partially outstanding).

**How to mark — do this once per ticket that has at least one Resolved AC:**
1. **Re-fetch the field fresh right before writing** (re-run the pre-review `curl` GET for that ticket) so you PATCH the latest value and don't clobber concurrent edits. The `Microsoft.VSTS.Common.AcceptanceCriteria` field is rich-text **HTML**.
2. **Edit only the resolved AC lines, minimally:**
   - **If the criteria use checkboxes** (the HTML contains `<input type="checkbox" …>` items — ADO's checklist format), mark each resolved item's checkbox checked by adding a `checked="checked"` attribute to its `<input type="checkbox">` element. Leave already-checked boxes untouched.
   - **If the criteria do not use checkboxes** (plain list items or paragraphs), prepend a green checkmark `✅ ` to the beginning of that criterion's text.
   - Change **nothing else** — do not touch unresolved criteria, reorder items, reword text, or alter the surrounding markup.
3. **Be idempotent.** Skip any criterion whose checkbox is already checked, or whose text already begins with `✅`, so re-running the skill after another push never double-marks or stacks icons.
4. **PATCH the field back** with a JSON-Patch request (the `Bash(curl *)` grant covers it; this is the step that requires the PAT to have Work Items **Write** scope). To keep the HTML (with its quotes, `$`, and backticks) off the command line, write the JSON-Patch body to a temp file with the **Write tool** (the frontmatter grants `Write(/tmp/*)`) and send it with `--data-binary @<file>`:
   - Body file `/tmp/cr_ado_ac_<ticket>.json` contains:
     ```
     [{"op":"add","path":"/fields/Microsoft.VSTS.Common.AcceptanceCriteria","value":"<full updated HTML>"}]
     ```
     (`op: "add"` on an existing field overwrites it in ADO's JSON-Patch semantics. The `value` must be the **entire** updated field HTML, not just the changed lines. JSON-escape the HTML properly when building the file.)
   - Send it:
     ```
     curl -sS -u ":$ADO_PAT" -X PATCH \
       -H "Content-Type: application/json-patch+json" \
       --data-binary @/tmp/cr_ado_ac_<ticket>.json \
       -w "%{http_code}" -o /tmp/cr_ado_ac_resp_<ticket>.json \
       "https://itsals.visualstudio.com/E_Retain_Content/_apis/wit/workitems/<ticket>?api-version=7.0"
     ```
   - **Never** print, echo, or write `$ADO_PAT` — always reference it as the variable.
5. **Detect write failures — don't report a false success.** Treat a `403`/`401`, or a `200`/`203` whose body is the sign-in HTML page (`<!DOCTYPE` / "Azure DevOps Services | Sign In"), as a permission failure: the PAT is missing Work Items **Write** scope (Read alone cannot update the ticket). For that ticket, note "ℹ️ Could not update AB#<ticket>'s acceptance criteria — the `ADO_PAT` needs Work Items **Write** scope (currently read-only or unauthorized). Grant write at https://itsals.visualstudio.com/_usersSettings/tokens and re-run." and continue to the next ticket. Only a `200` counts as success.
6. **Report per ticket** what changed, e.g. "🎫 Marked 2 of 3 acceptance criteria complete on AB#<ticket> (checked the boxes)." or "🎫 Prefixed ✅ to 1 acceptance criterion on AB#<ticket>." If a ticket had resolved ACs but they were already marked, say so ("🎫 AB#<ticket>: resolved criteria were already marked — no change.").

Never mark a criterion the diff did not actually resolve, and never mutate any ADO field other than `Microsoft.VSTS.Common.AcceptanceCriteria`. This is the only ADO write this skill ever performs.
